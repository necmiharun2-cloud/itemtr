...
const Checkout = () => {
  ...
  const loadUserBalance = useCallback(async () => {
    ...
    setUserBalance(Number(data?.balance ?? user.balance ?? 0));
  }, []);
  ...
  useEffect(() => {
    const onAuth = () => void loadUserBalance();
    const onVis = () => {
      if (document.visibilityState === "visible") void loadUserBalance();
    };
  ...
  useEffect(() => {
    ...
    if (listingId && isBotListingLocked(listingId)) {
      setBlockedReason("Ürün satışta değil");
      toast.error("Ürün satışta değil");
      return;
    }
    if (listingId && draft?.id === listingId && draft.isPurchasable !== false) {
      setCheckoutItem({ ...draft, seller: draft.seller.toUpperCase(), category: draft.category.toUpperCase() });
      return;
    }
    if (listingId) {
      const listing = getBotListingById(listingId);
      if (listing)
        setCheckoutItem({
          id: listing.id,
          title: listing.title,
          seller: listing.seller.toUpperCase(),
